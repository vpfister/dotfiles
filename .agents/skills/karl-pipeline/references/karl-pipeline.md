...
Set `JUPYTER_TOKEN` locally before launching JupyterLab.
...
- JupyterLab: `uv run --project zephyr --frozen --extra karl --with jupyterlab jupyter lab <notebook> --ServerApp.token="$JUPYTER_TOKEN" --no-browser --port=18888 --ServerApp.ip=0.0.0.0`
