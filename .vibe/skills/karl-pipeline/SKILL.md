---
name: karl-pipeline
description: KARL data synthesis pipeline (synthesize, solve, judge, filter, check, export). Use when the user wants to run the KARL pipeline, work with KARL data, export SFT/RL traces, run data_hunter checks, or analyze KARL experiment results.
---
...
Items discarded by the difficulty filter may have wrong reference answers. Run check on these to correct answers and rescue valid-but-hard questions:
...
After rescue, patch corrected answers and generate rubrics:
```bash
uv run --project zephyr --frozen --extra karl python \
...
**Important**: run `rescue_rubrics` AFTER check, because `output_rl.jsonl` was written by filter BEFORE check corrected answers.
...
- **Container**: `gpu_bc0fe89c.sqsh` with frozen code snapshot
...
- **Wall time**: 12h on dev QoS
- ChromaDB symlink: do NOT change `.chromadb` symlink while solve is running

## SLURM for KARL

CPU-only jobs (API calls). `dev` or `priority-ml4_taskforce` QoS:
...
3. **Register** (optional, for confstore): place in `/mnt/vast/datasets/finetuning/instruct/one_off/<name>/raw.jsonl`
4. **Write sweep YAML**: use `instruct:-1` path with raw file paths
5. **Key replay sources** (by raw path):
...
Model config for shrimp (MS4.1): `shrimpstral_mla` with `override_parameters_str: ["moe.expert_capacity_factor,moe.topk_first,attention_type"]`. Do NOT use `multimodal_24B_410M`.
